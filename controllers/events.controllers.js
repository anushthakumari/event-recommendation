...
} = require("../services/recom.services");
...
module.exports.find_event_bookings = async (req, res) => {
	...
	const algoType = req.query.type || "collaborative";

	try {
		const recom_events =
			algoType === "collaborative"
				? await runRecommendationSystem(userId)
				: await runContentBasedRecomm(userId);
...
module.exports.get_event_types = async (req, res) => {
	try {
		...
		console.error("Error geting events:", e);
		res.status(500).json({ message: "Internal Server Error" });
	}
};
