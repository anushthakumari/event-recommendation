...
const Layout = ({ children }) => {
	...
	return (
		<div>
			<nav className="relative px-4 py-2 flex justify-between items-center bg-white">
				...
				{user ? (
					<button
						...
						className="hidden lg:inline-block py-2 px-6 bg-red-500 hover:bg-blue-600 text-sm text-white font-bold rounded-xl transition duration-200"
						href="#">
						Log Out
					</button>
				) : null}
			...
			{user ? (
				<h1 className="px-4 capitalize text-lg">Hello, {user.name}</h1>
			) : (
				""
			)}
